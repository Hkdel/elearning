...
public class RoleServlet extends HttpServlet {
	...
	protected void grant(HttpServletRequest request,
		...
		List<Auth> authList = authDao.findAll();// 查询出所有 包括没有父级的功能
		Set<Auth> parentList = new HashSet<Auth>();// set可以去重
		for (Auth a : authList) {
			if (a.getParent().getId() == 0) {// 如果==0则是父级功能
				parentList.add(a);
			}
		}
	...
	protected void saveRoleAuth(HttpServletRequest request,
		...
		if(authIds == null){
			request.setAttribute("granterror", "您没有赋权，请赋权！");
	...
	protected void cancel(HttpServletRequest request,
		...
		if (f) {
			...
			String name = role.getName();
			request.setAttribute("role", role);
			request.setAttribute("cancelSuccess", "角色注销成功");
			...
			Role role = roleDao.getRoleById(id);
			String name = role.getName();
			request.setAttribute("cancelerror", "角色注销失败");
			request.getRequestDispatcher("role?method=list&name="+name).forward(request,
					response);
		}
	}
	...
	protected void restore(HttpServletRequest request,
		...
		if (f) {
			...
			String name = role.getName();
			request.setAttribute("role", role);
			request.setAttribute("restoreSuccess", "角色恢复成功");
			request.getRequestDispatcher("role?method=list&name="+name).forward(request,
					response);
		} else {
			HttpSession session = request.getSession();
			...
			Role role = roleDao.getRoleById(id);
			String name = role.getName();
			request.setAttribute("restoreerror", "角色恢复失败");
			request.getRequestDispatcher("role?method=list&name="+name).forward(request,
					response);
		}
	}
	...
	protected void update(HttpServletRequest request,
		...
		if (f) {
			PageUtils pageUtils = (PageUtils) session.getAttribute("pageUtils");
			int page = pageUtils.getCurrPage();
			request.setAttribute("updateSuccess", "角色修改成功");
			...
			PageUtils pageUtils = (PageUtils) session.getAttribute("pageUtils");
			int page = pageUtils.getCurrPage();
			request.setAttribute("updateerror", "角色修改失败");
	...
	protected void add(HttpServletRequest request,
		...
		if(roleName == null || "".equals(roleName)){
			request.setAttribute("error", "请输入角色名称！");
			...
			if (f) {
				...
				request.setAttribute("addSuccess", "角色添加成功");
				request.getRequestDispatcher("role?method=list&page="+page).forward(request,
						response);
			} else {
				request.setAttribute("adderror", "角色添加失败");
