...
public class ImgServlet extends HttpServlet {
		...
		try {
			...
			for (FileItem item : fileItems) {
				if (item.isFormField()) {// 这个是表单项
					...
					urlRealName = UUID.randomUUID().toString();// UUID.randomUUID().toString()是javaJDK提供的一个自动生成主键的方法。
			...
			if (f) {
				...
				request.setAttribute("addSuccess", "添加图片成功");
				request.getRequestDispatcher("img?method=list&page=" + page)
						.forward(request, response);
			} else {
				request.setAttribute("adderror", "添加图片失败！");
	...
	protected void delete(HttpServletRequest request,
		...
		if (f) {
			request.setAttribute("deleteSuccess", "删除图片成功！");
			request.getRequestDispatcher("img?method=list").forward(request,
					response);
		} else {
			request.setAttribute("deleteerror", "删除图片失败！");
			request.getRequestDispatcher("img?method=list").forward(request,
					response);
		}
	}
	...
	protected void update(HttpServletRequest request,
		...
		try {
			...
			for (FileItem item : fileItems) {
				if (item.isFormField()) {// 这个是表单项
					...
					if (newUrl != null && !"".equals(newUrl)) {
						String path = request.getRealPath("/photo");// 获取文件需要上传到的路径
						newUrlRealName = UUID.randomUUID().toString();// UUID.randomUUID().toString()是javaJDK提供的一个自动生成主键的方法。
			...
			if (f) {
				...
				request.setAttribute("updateSuccess", "修改图片成功");
				request.getRequestDispatcher("img?method=list&page=" + page)
						.forward(request, response);
			} else {
				request.setAttribute("updateerror", "修改图片失败！");
				request.getRequestDispatcher("img?method=list").forward(
						request, response);
			}
		} catch (FileUploadException e) {
